...
> i) if $a<c$ then  then  $c$  is the next smallest value of  $q$ . Further there are exacty 2 primitive vectors $vu\in \mathbb{Z}^{2}$  $s$ . $t$ . $a=q(u)$ namely $u=(\pm{1},0)$ and excatly two primitive vectors $v\in \mathbb{Z}^{2}$  $s$ . $t$ . $c=q(v)$ namely $v=(0,\pm 1)$  unless $a=b$ , in which case there are four vectores $v\in \mathbb{Z}^{2}$  $s$ . $t$ . $c=q(v)$ namely $v=(0,\pm 1),(1,-1),(-1,1)$ 
> ii) if $a=c$ then $a=q(u)$ has excatly four primitive solution $u\in \mathbb{Z}$ namely $u=(\pm 1,0),(0,\pm 1)$ unless $a=b=c$ in which case there are six solutions namely $(\pm 1,0)(0,\pm1)(-1,1)(1,-1)$ 
