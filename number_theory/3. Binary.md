...
3. $GL_{2}(\mathbb{Z})=$ set of all $2\times{2}$ Invertable Matrices note the determinant here is 1 or -1
...
> $-a<n\leq a<c$  or $0\leq b\leq a=c$ 
...
4aq(x,y) & =(2ax+by)^{2} +\mod[D]y^{2}  \\
 &  \geq4(4ac-b^{2} ) \\
\implies a(x,y) & \geq 4ac-b^{2}   \\
\text{Now, } -a< & b<a<c \text{we have} \\
b^{2}  & \leq ac  \\
\text{so, } aq(x,y) & \geq4ac-ac=3ax  \\
...
then $q(x,1)>c$  unless ($x=0$ ) or ($a=b$ and $x=-1$ )
 or 
 $q(x,-1)>0$  unless $x=0$ or($x=1$ and $a=b$ )
...
> $q=(q,b,c)$
> $q'=(a',b',c')$
> be reduced bqf of the same discriminant with $q \sim ^+q'$ then $q'=q, a=a',b=b',c=c'$  
...
so $(a',b',c')=qA=(a,b\pm2a\beta,)$
thus $b'=b\pm_{2}a\beta$
since $q,q'$ are reduced we have  $-a<b<a$ and $-a'<b'<a'$ 
$\implies -a<b\leq a \implies -a<b\pm 2a\beta<a$
$\implies -a\leq-b_{1}<a$ and $-a<b\pm{2}a\beta<a$ 
adding we get 
...
> then $q(x,y)=a or c$
> ie, $(x,y)=(\pm 1,0)$ or $(0,\pm1)$

`\begin{proof}`
we know that $q(x,y)>c$ for $|y|\geq2$ hence 
$a\leq q(x,y)\leq c\implies|y|=1$

(todo::complete the rest)

`\end{proof}`
> [!theorem] 
> Let  $q$  be a bqf of discriminant $D<0$ then there is a reduced bqf $(a,b,c)$ such that $q \sim^{+} (a,b,c)$

`\begin{proof}` 1
by Lagrange reduction lemma there is a form $(a,b,c)$ such that $q \sim^{+} (a,b,c)$ and $|b|\leq|a|\leq|c|$ since by convetion all values are positive we have $a,c>0$
$|b|\leq a\leq c$ now if $-a<b\leq a<c$ then we are done 
case I $a=c$ but b<0
in this case $(a,b,c) \underbrace{ \sim^{+}  }_{ \begin{bmatrix}0 & 1 \\ -1 & 0\end{bmatrix} }(a,-b,a)$

case II a< $c$  but b=-a
in this case 
$(a,b,c)\underbrace{ \sim^{+} }_{ \begin{bmatrix}1 & 1 \\  0& 1\end{bmatrix} }(a,-a,c)$
...
$$
\begin{align}
q_{1}=(a_{o},b_{0},c_ o ) &  \sim^{+} (a_{o},b_{0} ,c_{0})  \\
 & = (a_{1},b_{1},c_{1}) \text{ (say)} \\
\text{Note that } |a_{1} |+|b_{1} | & =|c_{o} |+|b_{o}|<|a_{1}|+|b_{1}| 
\end{align}
$$

Case II: $a_{0}\leq c_{0}$ but $b_{0}\not\in(-a_{0},a_{0}]$
							Sub-casesddddddddddddddddddddddddddddddddddddddcv A: $b_{0}>a_{0}$ 
then $(a_{0},b_{0},c_{0}) \underbrace{ sim^{+} }_{ \begin{bmatrix}1 & -1 \\ 0 & 1\end{bmatrix} }(a_{0},b_{0}-2a_{0})=(a_{1},b_{1},c_{1})$ say

$|a_{1}|+|b_{1}=|a_{0}|+|b_{0}-2a_{0}|<|a_{0}|+|b_{1}|$

sub-case  $B$ : $b_{0}<-a_{0}$
in this case 
$$
\begin{align}
(a_{0},b_{0},c_{0}) & \underbrace{ \sim^{+} }_{ \begin{bmatrix}1 & 1 \\ 0 & 1\end{bmatrix}  }(a_{0},b_{0}+2a_{0},c_{0}+b_{0}+a_{0})  \\
 & = (a_{1},b_{1},c_{1})
\end{align}
$$
here again $|a_{1}|+|b_{i}|< |a_{0}|+|b_0|$
Sub-case  $C$ : $b_{0}=-a_{0}$
here $(a_{0},b_{0},c_{0})=(a_{0},-a_{0},c_{0})\underbrace{ \sim^{+} }_{ \begin{bmatrix}1 & 1 \\ 0 & 1\end{bmatrix} }(a_{0},a_{0},c_{0})$
in this case the process completes 
In general continues this way we get a sequence 
$q_{1}=(a_{0},b_{0},c_{0})\sim^{+}(a_{1},b_{1},c_{1})\sim^{+}(a_{2},b_{2},c_{2}\dots)$
