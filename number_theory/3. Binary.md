...
> let $D\equiv 1,0 \pmod{4}$ let $p$ be a prime such that  $\left( \frac{D}{p} \right)=1$ Then there is a [BQF](#^ebdd1f) $q$ of the disc $D$ which properly represents $p$ . This form is unique up to equivalence
 ...
 We know that $q$ represents $p$ from [[#^f95cab]]  (question::How to prove it if  $D$  is congruent to 1 mod 4 as Lagrange theorem is only for square mod 4n ) 
...
\end{align}
$$
 (todo::why cant 4 divide one of it and the other be odd)  for both  $b$  and $b'$ should have the same parity or the congruence to 4 wont hold
