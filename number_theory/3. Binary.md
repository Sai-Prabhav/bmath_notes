...
\end{align}
$$
from [[bezout's theorem]] $\alpha x'+\beta y'=1$ then multiply from left by $\begin{bmatrix}\alpha \\ \beta\end{bmatrix}$ and by right from $[\alpha \ \beta]$ then $(x',y')$ and $\begin{pmatrix}x' \\ y'\end{pmatrix}$ gets canceled hence 
