...
>[!prp ]   
>Let $q=(a,b,c)$ be a [[#^ebdd1f]] with $\text{disk}(q)=D=b^{2}-4ac$
...
> [!corollary] 
> let $D\equiv 1,0 \pmod{4}$ let $p$ be a prime such thta $\left( \frac{D}{p} \right)=1$ Then there is a[[#^ebdd1f]] $q$ of the disc $D$ which properly represents $p$ 