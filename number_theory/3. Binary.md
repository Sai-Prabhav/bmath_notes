...
> $-a<n\leq a<c$  or m$0\leq b\leq a=c$ 
...
`\begin{proof}`
$q'=qA$ with $a=\begin{bmatrix}\alpha & \beta \\ \gamma & \delta\end{bmatrix}$ with $\alpha\delta-\beta\gamma=1$ since $q'\sim^{+} q$ they have the same set of values so in particular their smallest values are equal $a=a'$ 
case I: $a<c$ 
in this case $c=q(v)$ with exactly 2 primitive solution namely $v=(0,\pm 1)$ hence, $c=q'(w)$ , $a=a'=q'(w'=(x',y'))$also has exactly 2 solutions. now,
$q'(w)=q(\alpha x+\beta y,\gamma x+\delta y)$ where $w=(x,y)$ 
hence,
$$
\begin{align}
a' & = q'(\pm ,0 ) \\
 & =q'(\alpha x'+\beta y',\delta x'+\delta y') 
\end{align}
$$
there fore $w'=(\pm 1,0)$  $i$ .  $e$ , $x'=\pm 1,y'=0$
which implies 
$$
\begin{align}
\alpha x'+\beta y' & =\pm 1 \\
\alpha(\pm 1) & =\pm 1\implies\alpha=\pm 1 \\
\gamma & =0
\end{align}
$$
let $\alpha=+1$ 
also $\alpha\delta-\beta\gamma=1\implies\delta=1$ 
than $A=\begin{bmatrix}1 & \beta \\ 0 & 1\end{bmatrix}$
now $c=q(v)$ and $c=q'(w)$ have exactly two solutions each namely $v=(0,\pm 1)$ 
$$
\begin{align}
q'(w) & =q(x+\beta Y,Y)  \\
 &  =q(0,\pm1) \\
\text{thus } y & =\pm 1 \\
x\pm\beta & =0 \implies \beta=\pm x
\end{align}
$$
now let  $q'=q\cdot A=q \begin{pmatrix}1 & \beta \\ 0 & 1\end{pmatrix}$ 
 $i$ , $e$ . 
 $$
\begin{align}
			(a,b,c ) & =(a',b',c')\cdot\begin{pmatrix}1 & \beta \\ 0 & 1\end{pmatrix}  \\
 & =(a,b+2\beta a,c+a\beta^{2} +b\beta)
\end{align}
$$
since $q'$ is reduced we have $-a'<b'\leq a'$ 

$$
\begin{align}
\implies -a'< & b+2a'\beta\leq a' \\
\implies -a< & b+2a\beta<a_{1} \\
\text{also }-a< & b\leq a \to i \\
	\implies -a\leq & -b<a_{1} \to ii
\end{align}
$$
adding  $i$  and $ii$ 
$\implies -2a<2a\beta<2a\implies\beta=0$  
Case II: $a=c$ in this case 
$a=q(u)$  has exactly four solution, namely $u=(\pm1,0),(0,\pm1)$ 
`\end{proof}`
