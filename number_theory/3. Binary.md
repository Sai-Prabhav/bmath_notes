...
$$
> [!proposition]   
> let $q=(a,b,c)$ be a  [[#^ebdd1f]] with $\text{disk}(q)=D\leq0$.
...
#23-sep 
# Equivalence
1. $Q(\mathbb{Z}^{2})=$ set of all [[#^ebdd1f]] 
2. $M_{2}(\mathbb{Z})=$ set of all $2\times{2}$ Matrices
3. $GL_{2}(\mathbb{Z})=$ set of all $2\times{2}$ Invertable Matrices
...
\end{align}
$$
Define the new [[3. Binary#^ebdd1f]] $q.A$ by
$$
\begin{align}
...
$$
\begin{align}
q(x,y)=(x,y)\text{mat } q\begin{pmatrix}
x\\ y
\end{pmatrix}
\end{align}
$$
...
\end{align}
$$
Hence $\text{mat }q.A=A^{T}\cdot\text{mat }q \cdot A$ (question::can we be sure of it? ($x$,y) is not invertable)
> [!theorem] 
> let $q_{1}\sim q_{2}$
...
\end{align}
$$
since $\alpha\delta-\beta\gamma=1$ $\because A\in SL_{2}(\mathbb{Z})$ 
...
> 2. there is a [[3. Binary#^ebdd1f]]  $q$ of discriminant $D$ which properly represents $n$ 

1$\implies$ 2
say $b^{2}=D+4nc$ for some $c\in \mathbb{Z}$ $\implies b^{2}-4nc=D$
let $q=(n,b,c)$ then $q(1,0)=n$ so $q$ properly represents $n$ 
2 $\implies$ 1
let $q$ be a [[3. Binary#^ebdd1f]] of disc $D$ which properly represents $n$ then by [[3. Binary#^6cf17c]] 
$$
\begin{align}
q\sim^{+}(n,b,c) 
\end{align}
...
> [!corollary] 
> let $D\equiv 1,0 \pmod{4}$ let $p$ be a prime such thta $\left( \frac{D}{p} \right)=1$ Then there is a[[#^ebdd1f]] $q$ of the disc $D$ which properly represents $p$ . This form is unique up to equivalence

 `\begin{proof}`
 By Lagrange, there is a bqf $q$ which properly represents $p$. Then we know that 
 $$
\begin{align}
q \sim^{+} (p,b,c)
\end{align}
...
\end{align}
$$
if $p|b-b'$ then we are done $\implies b=b'$ 
so suppose $p \nmid b-b'$ but $p|b+b'$ 
$$
\begin{align}
\implies b+b'=2p \text{ or }0 \\
b=b'=p \text{ or } o
\end{align}
$$
so in any case $b=b'$
...
# Example
5 is an fundamental discriminant  for if $q=(a,b,c)$ is of discriminant 5 and if $d=\gcd(a,b,c)$ then $d ^{2}| b^{2}-4ac=5$ which implies $d=1$
