...
\end{align}
$$
let $d=\gcd(x,y)\implies x=dx',y=dy'$
$$
\begin{align}
(x',y')\text{mat }(q\cdot A)\begin{pmatrix}x' \\ y'\end{pmatrix}  & =(x',y')A^{T} (\text{mat }q)A\begin{pmatrix}x'\\ y' \end{pmatrix}  
\end{align}
$$
		from [[bezout's theorem]] $\alpha x'+\beta y'=1$ then multiply from left by $\begin{bmatrix}\alpha \\ \beta\end{bmatrix}$ and by left from $[\alpha \ \beta]$ then $(x',y')$ and $\begin{pmatrix}x' \\ y'\end{pmatrix}$ gets cancl
...
> [!corollary] 
> let $D\equiv 1,0 \pmod{4}$ let $p$ be a prime such thta $\left( \frac{D}{p} \right)=1$ Then there is a[BQF](#^ebdd1f)$q$ of the disc $D$ which properly represents $p$ . This form is unique up to equivalence
