...
then for any natural number  $n$ ,  $n$  is not bound 
...
thus $u- \frac{1}{n_{k}}\leq f(y_{n_{k}})\leq u$ 
	as  $k$  tends to $\infty$ by squeeze theorem $\left\{ f(x_{n_{k}}) \right\}$  is convergent and converges for $U$ as  $f$  is continus at  $d$  $\lim_{ k \to \infty }f(Y_{n_{k}})$ 
`\end{proof}`
