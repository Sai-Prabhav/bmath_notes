...
let $e=\inf D$ 
$c\leq e<e+\frac{1}{n} \quad \forall \ n>1$ 
as $e-\frac{1}{n}$ is not an upper bound there exit $y_{n} \st e\leq y_{n}\leq e+\frac{1}{n} \quad \forall \ n>1$ 
as $n\to \infty$ $y_{n}=e$ clearly $e=c$ and $f(y_{n})\geq0$
`\end{proof}`